%%%修士論文で使いました
%SVD,CAPZそれぞれのSISO制御結果(IIRフィルタ)とMIMO制御結果(MPAPフィルタ)を時間領域・周波数領域で表示
%比較のために、オリジナルのIRとFIRフィルタの制御結果も並べて表示

%測定したIRをSVDでモデル化。フィードバックを内包した逆フィルタ。プラントはパラメトリックなスピーカーモデル。非線形性を導入してロバスト性を確認。
%結局，フィードバックを含めても逆フィルタの対象にしているシステムが変動することには変わりないからロバスト性でない。
%逆フィルタの対象にするシステムを一定に保つためにフィードバックを使わないと。->iir_svd_Feedback_simulinkSpModel3
...
for noise = [0,0.3]
    ...
    %% CAPZモデル化
    %
    % [Ap,Bp,Cp,Dp] = capz(P,P-1,irSim); %CAPZモデル
    % sys_capz = ss(Ap,Bp,Cp,Dp,1/fs);
    % capz_model = impulse(sys_capz,time);
...
%         y_fir=ap(y_fir);
        ...
        %G = zeros(6,1);
        %feedback= zeros(1,6);
        open_system('SVD_IIR_Feedback_Sim_Cont_Press','loadonly')
        sim('SVD_IIR_Feedback_Sim_Cont_Press')
...
%%図の保存に使う%%
%filename=['IIR_tap50\f_r',num2str(k),];
%saveas(gcf,filename,'epsc')
...
set(gcf,'Position',[0 0 350 320])
%fftplot_kudoPhase(ap(ir(k,:)),fs,point,0,'k',1);hold on
phasePlot(IrPlant(1,:),fs,'b');%iir
hold all
phasePlot(IrPlant(2,:),fs,'b--');

%fftplot_kudoPhase(y_fir,fs,point,0,'r',1);%iir
%fftplot_kudoPhase(capz_iir,fs,point,0,'b--',1);%iir
phasePlot(IrSvd(1,:),fs,'g',2);%iir
phasePlot(IrSvd(2,:),fs,'g--',2);
phasePlot(IrSvdFb(1,:),fs,'r');%iir
phasePlot(IrSvdFb(2,:),fs,'r--');
...
if SignalChange == 0
    fftPlot2(IrPlant(1,:),fsSim,'k');
    hold on
    %fftPlot2(IrPlant(2,:),fsSim,'k--');

    set(gcf,'Position',[0 0 640 520])

    fftPlot2(IrSvd(1,:),fsSim,'k',2.5);
    %fftPlot2(IrSvd(2,:),fsSim,'k--',1.5);

    fftPlotMarker(IrSvdFb(1,:),fsSim,'k--',1.5);
    %fftPlotMarker(IrSvdFb(2,:),fsSim,'k--',2.5);
    ...
    xlabel('Frequency[Hz]','FontSize',12)
    ylabel('Relative SPL[dB]','FontSize',12)
    leg=legend('Original','Original with noise','Inverse Filtering','Inverse Filtering with noise','Inverse Filtering + Feedback','Inverse Filtering + Feedback with Noise','Location','Southeast');%iir
    set(leg,'FontSize',10)
else
    set(gcf,'Position',[0 0 640 800])
    subplot(311)
    fftPlot(IrPlant(1,:),fsSim,'k');
    axis([fInput-10 250 -60 0])
    xlabel('Frequency[Hz]','FontSize',12)
    ylabel('Relative SPL[dB]','FontSize',12)
    legend('Original')
    ...
    subplot(312)
    fftPlot(IrSvd(1,:),fsSim,'k');
    axis([fInput-10 250 -60 0])
    xlabel('Frequency[Hz]','FontSize',12)
    ylabel('Relative SPL[dB]','FontSize',12)
    legend('Inverse Filtering')
    grid on

    subplot(313)
    fftPlot(IrSvdFb(1,:),fsSim,'k');
    xlabel('Frequency[Hz]','FontSize',12)
    ylabel('Relative SPL[dB]','FontSize',12)
    legend('Inverse Filtering + Feedback')
    grid on
...
%% 標準偏差の計算
F2 = 100;
sdPlant1 = fft_sd(IrPlant(1,:),fs,20,F2);
sdPlant2 = fft_sd(IrPlant(2,:),fs,20,F2);
sdSvd1 = fft_sd(IrSvd(1,:),fs,20,F2);
sdSvd2 = fft_sd(IrSvd(2,:),fs,20,F2);
sdSvdFb1 = fft_sd(IrSvdFb(1,:),fs,20,F2);
sdSvdFb2 = fft_sd(IrSvdFb(2,:),fs,20,F2);
